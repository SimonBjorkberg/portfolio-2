...
export default function ProjectContainer({ project, setX }: Props) {
    ...
    return (
        <>
            <div onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)} onClick={() => { setSelectedProject(project); setX(true) }} className={`hidden md:flex flex-col max-w-[250px] rounded-md p-5 h-[220px] bg-[#121f27] hover:bg-[#172530] hover:cursor-pointer transition-all duration-300 justify-between`}>
